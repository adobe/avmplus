...
#define SIBIDX(n)    "1248"[n]
    ...
    */
    ...
    bool RegAlloc::canRemat(LIns* ins)
    {
        ...
        // TODO: Contrary to the comment above, we can blind imm32 using LEA, which
        // does not affect the CCs.
        return (ins->isImmAny() && !(ins->isImmI() && ins->isTainted() && shouldBlind(ins->immI()))) || ins->isop(LIR_allocp) || canRematLEA(ins);
    }
    ...
    void Assembler::asm_store32(LOpcode op, LIns* value, int dr, LIns* base, bool tainted)
    {
        ...
        if (value->isImmI() && !(value->isTainted() && shouldBlind(value->immI()))) {
            ...
            if (base->opcode() == LIR_addp && !force && !(tainted && shouldBlindDisplacement(dr))) {
                LIns* index;
                int scale;
                ...
                int c = value->immI();
				Register orb;
				Register rb = getBaseRegWithBlinding(base, dr, GpRegs, tainted, force, &orb);
				switch (op) {
            ...
            if (base->opcode() == LIR_addp && !force && !(tainted && shouldBlindDisplacement(dr))) {
                ...
                if (base->isImmI()) {
					...
					Register orb;
                    getBaseReg2WithBlinding(SrcRegs, value, rv, GpRegs, base, rb, dr, tainted, force, &orb);
					switch (op) {
					case LIR_sti2c: ST8( rb, dr, rv);   break;
					case LIR_sti2s: ST16(rb, dr, rv);   break;
					case LIR_sti:   ST(  rb, dr, rv);   break;
    ...
    void Assembler::asm_load128(LIns* ins)
    {
        ...
        if (base->opcode() == LIR_addp && !force && !(ins->isTainted() && shouldBlindDisplacement(d))) {
            LIns* index;
            int scale;
            getBaseIndexScale(base, &base, &index, &scale);
            ...
            Register orb;
            Register rb = getBaseRegWithBlinding(base, d, GpRegs, ins->isTainted(), force, &orb);
            SSE_LDUPS(rr, d, rb);
            SSE_XORPDr(rr, rr);
            adjustBaseRegForBlinding(rb, orb);
        }
    ...
    void Assembler::asm_load64(LIns* ins)
    {
        ...
        if (ins->isInReg()) {
            ...
            if (base->opcode() == LIR_addp && (rmask(rr) & XmmRegs) && !force && !(ins->isTainted() && shouldBlindDisplacement(d))) {
                LIns* index;
                int scale;
                getBaseIndexScale(base, &base, &index, &scale);
            ...
            case LIR_ldd:
				if (_config.i386_sse2) {
					...
					Register orb;
					Register rb = getBaseRegWithBlinding(base, d, GpRegs, ins->isTainted(), force, &orb);
					NanoAssert((GpRegs & XmmRegs) == 0);  // correctness of allocTempReg below relies on this
					...
					if (force || (ins->isTainted() && shouldBlindDisplacement(d))) {
						d += _blindMask32;
						// We avoid copying via the FP stack because it's slow and likely to cause spills.
						Register t, trb;
						_allocator.allocTempReg2(GpRegs & ~rmask(rb), t, GpRegs & ~rmask(rb), trb);
						ST(FP, dr+4, t);
						LD(t, d+4, trb);
						ST(FP, dr, t);
						LD(t, d, trb);
    ...
    void Assembler::asm_store64(LOpcode op, LIns* value, int d, LIns* base, bool tainted)
    {
        ...
        if (op == LIR_std2f || (op == LIR_stf && (!value->isImmF() || value->isInReg()))) {
			...
			adjustBaseRegForBlinding(rb, orb);
        } else if (base->opcode() == LIR_addp && _config.i386_sse2 && !force && !(tainted && shouldBlindDisplacement(d))) {
            LIns* index;
            int scale;
            getBaseIndexScale(base, &base, &index, &scale);
    ...
    void Assembler::asm_load32(LIns* ins)
    {
        ...
        if (base->isImmI() && !force && !(ins->isTainted() && shouldBlindDisplacement(d))) {
            // We assume that the base address is never tainted, but might be predictable.
            NanoAssert(!base->isTainted());
        ...
        } else if (base->opcode() == LIR_addp && !force && !(ins->isTainted() && shouldBlindDisplacement(d))) {
            LIns* index;
            int scale;
            getBaseIndexScale(base, &base, &index, &scale);
            ...
            Register ora;
            Register ra = getBaseRegWithBlindingUsingSpecifiedTemp(base, d, GpRegs, ins->isTainted(), force, &ora, rr);
            switch (op) {
